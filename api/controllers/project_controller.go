...
// ShowProjects ...
// show all exists projects or 404 error
func ShowProjects(c *fiber.Ctx) {
	// Create DB connection
	db, errConnectDB := stores.OpenStore()
	if errConnectDB != nil {
		// DB connection error
	...
	if errGetProjects != nil {
		// Users not found
		c.Status(404).JSON(fiber.Map{
			"error": true, "msg": "projects not found", "count": 0, "projects": nil,
		})
		return
	}

	c.JSON(fiber.Map{"error": false, "msg": nil, "count": len(projects), "projects": projects})
}
